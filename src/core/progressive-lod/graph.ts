  ...
  const builder = new StateGraph(ProgressiveState)
    ...
    .addNode("scout", async (state: ProgressiveLodState, config?: RunnableConfig) => {
      ...
      const result = await runtime(config).call({
        ...
        session: sessionDirective(active.scoutSessionId, active.scoutSessionMode, active.scoutTurns), schema: z.toJSONSchema(ResearchSchema) as Record<string, unknown>,
      ...
      if (result.budgetStop) return { plan, usage, callsUsed, pendingBudget: { scope: "call" as const, role: "scout" as const, nodeId: active.id, stop: result.budgetStop, sessionId: result.sessionId }, resumeRole: "scout" as const, phase: "budget-paused" };
      const merged = mergeResearch({ ...state, plan }, groundedResearch(structured(result, ResearchSchema, "scout"), result));
      return { ...merged, plan, usage, callsUsed, resumeRole: "decider" as const, phase: "deciding" };
    })
    .addNode("decide", async (state: ProgressiveLodState, config?: RunnableConfig) => {
      ...
      const result = await runtime(config).call({
        ...
        session: state.deciderSessionId ? { strategy: "continue", sessionId: state.deciderSessionId } : { strategy: "fresh" }, schema: z.toJSONSchema(DetailDecisionSchema) as Record<string, unknown>,
      ...
      const usage = addUsage(state.usage, result.usage); const callsUsed = state.callsUsed + 1;
      if (result.budgetStop) return { usage, callsUsed, deciderSessionId: result.sessionId, pendingBudget: { scope: "call" as const, role: "decider" as const, nodeId: active.id, stop: result.budgetStop, sessionId: result.sessionId }, resumeRole: "decider" as const, phase: "budget-paused" };
      return { decision: structured(result, DetailDecisionSchema, "decide"), deciderSessionId: undefined, usage, callsUsed, phase: "merging" };
    })
    ...
    .addNode("budget_interrupt", (state: ProgressiveLodState) => {
      ...
      return { pendingBudget: undefined, budgetGrants, constraints, resumeRole: pending.role, phase: `resuming-${pending.role}` };
    })
    ...
    .addNode("implement", async (state: ProgressiveLodState, config?: RunnableConfig) => {
      ...
      if (result.budgetStop) return { usage, callsUsed, implementationSessions: sessions, pendingBudget: { scope: "call" as const, role: "implementer" as const, nodeId: leaf.id, stop: result.budgetStop, sessionId: result.sessionId }, resumeRole: "implementer" as const, phase: "budget-paused" };
      const output = structured(result, ImplementationResultSchema, "implement");
      return { usage, callsUsed, implementationSessions: sessions, implementationResults: { ...state.implementationResults, [leaf.id]: output }, activeLeafId: undefined, plan: state.plan.map((node) => node.id === leaf.id ? { ...node, status: output.status === "completed" ? "implemented" as const : "failed" as const } : node), phase: output.status === "completed" ? "selecting-leaf" : "implementation-blocked" };
    })
    ...
    .addNode("verify", async (state: ProgressiveLodState, config?: RunnableConfig) => {
      const result = await runtime(config).call({ agent: verifier, node: "verify", state, limits: state.roleLimits.verifier, session: state.verifierSessionId ? { strategy: "continue", sessionId: state.verifierSessionId } : { strategy: "fresh" }, schema: z.toJSONSchema(VerificationSchema) as Record<string, unknown>, prompt: `Task type: one aggregate independent verification. Inspect the actual worktree and diff. Check every implemented leaf against its contract and artifacts. Do not edit files. failedNodeIds must be exact plan IDs.\n\n${JSON.stringify({ task: state.originalTask, constraints: state.constraints, leaves: state.plan.filter((node) => node.leaf).map((node) => ({ node, result: state.implementationResults[node.id] })) })}` });
      const usage = addUsage(state.usage, result.usage); const callsUsed = state.callsUsed + 1;
      if (result.budgetStop) return { usage, callsUsed, verifierSessionId: result.sessionId, pendingBudget: { scope: "call" as const, role: "verifier" as const, stop: result.budgetStop, sessionId: result.sessionId }, resumeRole: "verifier" as const, phase: "budget-paused" };
      const verification = structured(result, VerificationSchema, "verify");
      return { usage, callsUsed, verifierSessionId: result.sessionId, verification, plan: applyVerification(state.plan, verification), phase: verification.passed ? "completed" : "verification-failed" };
    })
    ...
    .addNode("repair", async (state: ProgressiveLodState, config?: RunnableConfig) => {
      ...
      const result = await runtime(config).call({ agent: repairAgent, node: `repair:${leaf.id}`, state, limits: state.roleLimits.repair, session: { strategy: "continue", sessionId }, schema: z.toJSONSchema(ImplementationResultSchema) as Record<string, unknown>, prompt: `Task type: bounded repair of the same cohesive leaf. Address only the verifier findings for this leaf and rerun focused checks. Return structured artifacts.\n\n${JSON.stringify({ leaf: leafPayload(state, leaf), prior: state.implementationResults[leaf.id], verification: state.verification })}` });
      const usage = addUsage(state.usage, result.usage); const callsUsed = state.callsUsed + 1;
      if (result.budgetStop) return { usage, callsUsed, pendingBudget: { scope: "call" as const, role: "repair" as const, nodeId: leaf.id, stop: result.budgetStop, sessionId: result.sessionId }, resumeRole: "repair" as const, phase: "budget-paused" };
      const output = structured(result, ImplementationResultSchema, "repair");
      return { usage, callsUsed, repairAttempts: state.repairAttempts + 1, implementationResults: { ...state.implementationResults, [leaf.id]: output }, activeLeafId: undefined, plan: state.plan.map((node) => node.id === leaf.id ? { ...node, status: output.status === "completed" ? "implemented" as const : "failed" as const } : node), phase: output.status === "completed" ? "selecting-repair" : "implementation-blocked" };
    })
