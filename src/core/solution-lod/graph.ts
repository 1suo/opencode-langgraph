...
export function projectActivationContext(state: SolutionLodState, activation: Activation): Record<string, unknown> {
  ...
  if (activation.capability === "inspect") {
    return { ...common, earlierChoices, questionToAnswer: activation.request, permittedNextRequest: [], mustNotChooseSolution: true, outputRule: "reuse supplied fact IDs through factIds; put only genuinely new observations in evidence; omit region.objective entirely" };
  }
