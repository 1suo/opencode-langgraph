...
export function projectActivationContext(state: SolutionLodState, activation: Activation): Record<string, unknown> {
  ...
  if (activation.capability === "inspect") {
    const terms = new Set(`${activation.request} ${region.objective}`.toLowerCase().match(/[a-z0-9_-]{4,}/g) ?? []);
    const availableGraphFacts = state.network.evidence.filter((item) => isConfirmedEvidence(state.network, item.id)).map((item) => ({ item, score: (`${item.text} ${item.source}`.toLowerCase().match(/[a-z0-9_-]{4,}/g) ?? []).filter((term) => terms.has(term)).length })).filter(({ score }) => score > 0).sort((left, right) => right.score - left.score || left.item.id.localeCompare(right.item.id)).slice(0, 20).map(({ item: { id, text, source } }) => ({ factId: id, fact: text, source }));
    return { ...common, earlierChoices, availableGraphFacts, questionToAnswer: activation.request, permittedNextRequest: [], mustNotChooseSolution: true, outputRule: "reuse matching availableGraphFacts through factIds; put only genuinely new observations in evidence; omit region.objective entirely" };
  }
...
export function compileActivationPrompt(state: SolutionLodState, activation: Activation): string {
  ...
  const policy: Record<Capability, string[]> = {
    inspect: ["Answer the named repository question only.", "Search AVAILABLE GRAPH FACTS first. Reuse matching IDs in factIds and do not repeat their text/source as evidence.", "New inference is always a hypothesis. To validate a supplied hypothesis, return its claimRef, verdict, independent evidenceRefs, and reason in validations. You cannot confirm your own inference by labeling it confirmed."],
  ...
  const roleSections: Record<Capability, string[]> = {
    inspect: [section("AVAILABLE GRAPH FACTS", context.availableGraphFacts), section("QUESTION TO ANSWER", context.questionToAnswer), section("INSPECTION OUTPUT LIMIT", context.outputRule)],
