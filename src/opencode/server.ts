...
async function executeGraph(plugin: PluginInput, input: ExecuteGraphInput): Promise<GraphExecution> {
  ...
  const saved: StoredRun = { checkpointVersion: graphName === "solution-lod" ? 4 : undefined, runId, rootSessionId: input.rootSessionId, userMessageId: input.userMessageId, graph: graphName, task: input.task, directory: input.directory, worktree: input.worktree, modelAssignments: input.modelAssignments, status: "running" };
...
async function resumeFromCheckpoint(
  ...
  if (saved.graph === "solution-lod" && saved.checkpointVersion !== 4) throw new Error("This interrupted solution-lod run uses an incompatible checkpoint schema. Start a new message to create a clean state-v4 run.");
  const loaded = await loadConnectorDefinition(saved.worktree);
  const definition = saved.graph === "solution-lod" ? withSolutionRoleModelAssignments(loaded, saved.modelAssignments) : loaded;
  assertValidConnector(await validateConnector(definition));
  const configured = definition.graphs[saved.graph];
