...
11. A genuine decision domain contains one to seven materially distinct candidates — exactly one only when the boundary truly admits no materially different alternative — and must pass a fresh, fingerprint-bound challenge before selection or singleton collapse.
...
- **Inspect.** `langgraph_inspect` returns stored status, phase, result, usage, and a compact region tree (per-node status, domain phase, viable/total candidates, selection, blocked reason — the same metadata as the TUI left panel) plus compact run telemetry, all without mutation. The full semantic network is not included by default because it floods agent sessions; pass `regionId` to drill into one region's candidates, constraints, facts, activations, and artifacts, or `verbose: true` when the whole semantic network is strictly required. A queued or running run that has not reached its first checkpoint reports `no-checkpoint-yet` instead of failing. `/graph` shows the same semantic state in the TUI. All intermediate `schedule`/`activate` checkpoints are reachable this way.
