...
describe("solution LOD state v8 lifecycle", () => {
  ...
  it("runs inspect through all three synthesis operations, certified implementation, and verification", async () => {
    ...
    const runtime = { call: async (input: any) => {
      ...
      if (input.node === "refine:r1") return { text: "", structured: { evidence: [], children: [], certifiedLeaf: { implementationScope: "one bounded source edit", criterionIds: ["criterion:scope:r1:0"], evidenceRefs: [], mutationResources: ["src/feature.ts"], checks: [{ criterionId: "criterion:scope:r1:0", commandOrObservation: "run focused test" }] }, activations: [] } };
